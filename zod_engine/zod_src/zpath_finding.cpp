...
void ZPath_Finding_Engine::DeleteTileInfo()
{
	//we must first stop all path finding threads
	Clear_Thread_List();

	map_pathfinding_info_tile **delete_list;

	delete_list = path_robot_tile;
	if(delete_list)
	{
		for(int i=0;i<w;i++)
			free(delete_list[i]);
...
void ZPath_Finding_Engine::AllocTileInfo()
{
	int i, j;
...
void ZPath_Finding_Engine::SetImpassable(int x, int y, bool impassable)
{
	if(x < 0) return;
	if(y < 0) return;
	if(x >= w) return;
...
bool ZPath_Finding_Engine::WithinImpassable(int x, int y, int w, int h, bool is_robot)
{
...
int ZPath_Finding_Engine::Find_Path(int sx, int sy, int ex, int ey, bool is_robot, int obj_ref_id )
{
	ZPath_Finding_Response *response;
	...
	//direct possible?
	if(Direct_Path_Possible(sx, sy, ex, ey, is_robot)) return 0;
...
bool ZPath_Finding_Engine::Direct_Path_Possible(int sx, int sy, int ex, int ey, bool is_robot)
{
