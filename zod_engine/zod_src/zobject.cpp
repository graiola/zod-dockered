...
//Mix_Chunk *ZObject::selected_wav[6];
//Mix_Chunk *ZObject::selected_robot_wav[MAX_ROBOT_TYPES];
//Mix_Chunk *ZObject::acknowledge_wav[12];
...
void ZObject::SetEffectList(vector<ZEffect*> *effect_list_)
{
	effect_list = effect_list_;
}

void ZObject::SetDamageMissileList(vector<damage_missile> *damage_missile_list_)
{
...
bool ZObject::WithinAutoEnterRadius(int ox, int oy)
{
	...
	//int x = loc.x + (width_pix >> 1);
	//int y = loc.y + (height_pix >> 1);
	int &x = center_x;
	int &y = center_y;
	...
	////quick prelim tests
	//if(ox < x - ae_radius) return false;
	//if(ox > x + ae_radius) return false;
	//if(oy < y - ae_radius) return false;
	//if(oy > y + ae_radius) return false;
	...
	//int dx = x - ox;
	//int dy = y - oy;
	//if(sqrt((float)((dx * dx) + (dy * dy))) > ae_radius) return false;
...
bool ZObject::WithinAutoGrabFlagRadius(int ox, int oy)
{
	...
	////quick prelim tests
	//if(ox < x - agf_radius) return false;
	//if(ox > x + agf_radius) return false;
	//if(oy < y - agf_radius) return false;
	//if(oy > y + agf_radius) return false;
	...
	//int dx = x - ox;
	//int dy = y - oy;
	//if(sqrt((float)((dx * dx) + (dy * dy))) > agf_radius) return false;
...
bool ZObject::WithinAgroRadius(int ox, int oy)
{
	...
	////quick prelim tests
	//if(ox < x - agro_radius) return false;
	//if(ox > x + agro_radius) return false;
	//if(oy < y - agro_radius) return false;
	//if(oy > y + agro_radius) return false;
	...
	//int dx = x - ox;
	//int dy = y - oy;
	//if(sqrt((float)((dx * dx) + (dy * dy))) > agro_radius) return false;
...
bool ZObject::WithinAttackRadius(int ox, int oy)
{
	...
	////quick prelim tests
	//if(ox < x - attack_radius) return false;
	//if(ox > x + attack_radius) return false;
	//if(oy < y - attack_radius) return false;
	//if(oy > y + attack_radius) return false;
	...
	//int dx = x - ox;
	//int dy = y - oy;
	//if(sqrt((float)((dx * dx) + (dy * dy))) > attack_radius) return false;
...
int ZObject::ProcessServer(ZMap &tmap, ZOLists &ols)
{
	...
	//it being repaired?
	sflags.auto_repair = DoAutoRepair(tmap);
...
void ZObject::ProcessAttackDamage(ZMap &tmap, bool attack_player_given)
{
	...
	//if(attack_object->IsDestroyed())
	//{
	//	Disengage();
	//	return;
	//}

	////did they become our team?
	//if(attack_object->GetOwner() == owner)
	//{
	//	Disengage();
	//	return;
	//}

	////we still have explosives?
	//if(!HasExplosives() && attack_object->AttackedOnlyByExplosives())
	//{
	//	Disengage();
	//	return;
	//}
	...
	//can_attack_with_grenades = false;
...
void ZObject::ProcessMoveWP(vector<waypoint>::iterator &wp, double time_dif, bool is_new, ZOLists &ols, ZMap &tmap, bool stoppable)
{
	...
	//did we halt movement?
	if(!ProcessMove(time_dif, tmap, stoppable))
	{
		KillWP(wp);
		return;
	}
...
bool ZObject::ProcessMove(double time_dif, ZMap &tmap, bool stoppable)
{
	int &x = loc.x;
	int &y = loc.y;
	float &dx = loc.dx;
	...
	if(stoppable && tmap.WithinImpassable(inx+1, iny+1, width_pix-2, height_pix-2, object_type == ROBOT_OBJECT))
	{
		if(ReachedTarget())
			return true;
		else
...
bool ZObject::SetBuildingDefaultProduction()
{
	return false;
}

bool ZObject::SetBuildingProduction(unsigned char ot, unsigned char oid)
{
	return false;
}

void ZObject::CreateBuiltCannonData(char *&data, int &size)
{
	data = NULL;
	size = 0;
}
...
void ZObject::CreateBuildingStateData(char *&data, int &size)
{
	data = NULL;
	size = 0;
}
...
bool ZObject::StopBuildingProduction()
{
	return false;
}
...
bool ZObject::DoAutoRepair(ZMap &tmap)
{
	double &the_time = ztime->ztime;
...
void ZObject::CreateRepairAnimData(char *&data, int &size, bool play_sound)
{
	data = NULL;
	size = 0;
}
