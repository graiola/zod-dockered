...
enum map_editor_mode
{
	...
	PLACE_VEHICLE_MODE, PLACE_ROBOT_MODE, PLACE_ANIMAL_MODE,
...
{
	"place_tile", "place_building", "place_cannon", "place_vehicle", "place_robot", "place_animal", "place_item", "place_zone", "remove_zone", "remove_object"
};
...
void process_mouse_click(int x, int y)
{
	...
	if(x < 320 && y < 384)
	{
		...
		if(current_ptile != new_tile)
		{
			current_ptile = new_tile;
			draw_palette();
			SDL_Flip(screen);
		}
	}
			...
			switch(current_mode)
			{
				...
				case PLACE_BUILDING_MODE:
					...
					if(do_it)
					{
						...
						new_object.owner = current_team;
						new_object.object_type = BUILDING_OBJECT;
						new_object.object_id = current_object;
						new_object.blevel = current_blevel;
						new_object.extra_links = current_extra_links;
						new_object.health_percent = current_health_percent;
				...
				case PLACE_ZONE_MODE:
					...
					else
					{
						...
						//add it
						new_formal_zone.x = new_zone.x;
						new_formal_zone.y = new_zone.y;
						new_formal_zone.w = new_zone.w;
						new_formal_zone.h = new_zone.h;
...
void draw_info(bool flip)
{
	...
	blit_message(message, 5, 390+j, 255, 255, 255);
	
	j += 10;
	switch(current_mode)
	{
		case PLACE_TILE_MODE:
			sprintf(message,"Current Object: Palette Tile %d", current_ptile);
			break;
		case PLACE_VEHICLE_MODE:
