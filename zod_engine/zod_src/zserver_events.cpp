...
void ZServer::send_object_list_event(ZServer *p, char *data, int size, int player)
{
	for(vector<ZObject*>::iterator i=p->object_list.begin(); i!=p->object_list.end(); i++)
	{
		...
		//{
		//	(*i)->GetCords(object_info.x, object_info.y);
		//	(*i)->GetObjectID(object_info.object_type, object_info.object_id);
		//	object_info.owner = (*i)->GetOwner();
		//	object_info.ref_id = (*i)->GetRefID();
		//	object_info.blevel = (*i)->GetLevel();
		//	object_info.extra_links = (*i)->GetExtraLinks();
		//	object_info.health = (*i)->GetHealth();

		//	//send it
		//	p->server_socket.SendMessage(player, ADD_NEW_OBJECT, (char*)&object_info, sizeof(object_init_packet));
		//}
		...
		//send health
		{
			...
			send_data.ref_id = (*i)->GetRefID();
			send_data.health = (*i)->GetHealth();

			p->server_socket.SendMessage(player, UPDATE_HEALTH, (const char*)&send_data, sizeof(object_health_packet));
		}
		...
		//send building state
		{
			...
			(*i)->CreateBuildingStateData(data, size);
			if(data)
			{
				p->server_socket.SendMessage(player, SET_BUILDING_STATE, data, size);
				free(data);
			}
		}

		//send building repair anim info
		{
			char *data;
			int size;

			(*i)->CreateRepairAnimData(data, size, false);
			if(data)
			{
				p->server_socket.SendMessage(player, SET_REPAIR_ANIM, data, size);
				free(data);
			}
		}
		...
		//send group information
		{
			...
			if(data)
			{
				p->server_socket.SendMessage(player, OBJECT_GROUP_INFO, data, size);
				free(data);
			}
		}
