...
void selection_info::SetupGroupDetails()
{
...
void ZPlayer::PlayBuildingSounds()
{
	...
	bool robot_rendered = false;
	bool vehicle_rendered = false;
	...
	for(vector<ZObject*>::iterator i=object_list.begin(); i!=object_list.end(); i++)
	{
		...
		if(oid == RADAR && (*i)->GetOwner() != NULL_TEAM)
		{
			...
			if(zmap.WithinView(x, y, w, h)) radar_rendered = true;
		}
...
void ZPlayer::AddDevWayPointToSelected()
{
	...
	//map locations
	new_waypoint.x = mouse_x + shift_x;
	new_waypoint.y = mouse_y + shift_y;
	...
	if(lctrl_down || rctrl_down) new_waypoint.attack_to = true;
	...
	if(gui_window && gui_window->GetBuildingObj() && !select_info.selected_list.size())
	{
		...
		new_rallypoint.x = mouse_x + shift_x;
		new_rallypoint.y = mouse_y + shift_y;
...
void ZPlayer::SendDevWayPointsOfSelected()
{
	...
	for(i=select_info.selected_list.begin(); i!=select_info.selected_list.end(); i++)
	{
		...
		CreateWaypointSendData((*i)->GetRefID(), (*i)->GetWayPointDevList(), data, size);
...
void ZPlayer::SendLogin()
{
	...
	{
		//if not sending a login
		//then ask if we need to display the login menu
