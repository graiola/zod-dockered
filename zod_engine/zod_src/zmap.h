...
class ZMap
{
	public:
		...
		bool Loaded();
		bool PermStamp(int x_, int y_, ZSDL_Surface *surface);
		bool PermStamp(int x_, int y_, SDL_Surface *surface);
		...
		inline void SetImpassable(int x, int y, bool impassable = true)
			{ path_finder.SetImpassable(x, y, impassable); }
		inline bool WithinImpassable(int x, int y, int w, int h, bool is_robot)
			{ return path_finder.WithinImpassable(x, y, w, h, is_robot); }
